...
from app.dependencies.dep import get_db, require_admin
...
def create_room(room: RoomCreate, db: Session = Depends(get_db), _admin=Depends(require_admin)):
...
def get_rooms(
    capacity: int | None = None, location: str | None = None, db: Session = Depends(get_db)
):
    query = db.query(Room)
